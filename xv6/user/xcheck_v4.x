...
// unused | superblock | inode table(array of inode) | bitmap (data) | data blocks
...
//	int blockAddr, type;
	...
	// Number of links/reference counts to inode in file system
		...
		if (type >= 0 && type <= 3) {
			...
			if (type != 0) {
				// If directory, check to make sure it is a valid directory
		        	if (type == 1) {
		            		currDirectory = (struct dirent*)(img_ptr + dip->addrs[0] * BSIZE);
			...
			//			int x, y, z;
							...
							currDir = (struct dirent *)(img_ptr + parentDir->addrs[x]*BSIZE);
							// Check for valid parent directory
			                		for (int y = 0; y < BSIZE/sizeof(struct dirent); y++) {
								if (currDir->inum != i) { 
									//find this file in the parent directory list
									validParentDir = TRUE;
								}
								currDir++;
			                		}
		                		...
		                		if (dip->addrs[x] != 0) {
			                		currDir = (struct dirent *)(img_ptr + dip->addrs[x]*BSIZE);
							// Find used inodes and mark them in list
			                		for (int z = 0; z < BSIZE/sizeof(struct dirent *); x++) {
			                    			if (currDir->inum == 0) {
									break;
								}
					...
					int blockAddr = dip->addrs[k];
					// Make sure address is valid
		            		if (blockAddr != 0) {
						// Check 2: bad address in inode
				            	if((blockAddr) < ((int)BBLOCK(sb->nblocks, sb->ninodes))+1) {
				                	//TODO: indrect and indirect
							fprintf(stderr, "ERROR: bad direct address in inode.\n");
				                	exit(1);
				            	}
						if (blockAddr > (sb->size * BSIZE)) {
							fprintf(stderr, "ERROR: bad direct address in inode.\n");
                                                        exit(1);
						}
						// Check 7&8: check used blocks are only used once
						if(usedBlocks[blockAddr] == 1){
							//TODO: direct and indirect
